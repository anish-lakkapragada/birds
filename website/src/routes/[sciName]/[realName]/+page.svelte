<script>
  ...
  const {
    imageUrl,
    wikipediaImageURL,
    nuthatchDataAvailable,
    sciName,
    realName,
    description,
    nuthatchData,
    wikipediaDataAvailable,
    wikipediaPageHTML,
  } = data;
  ...
  //
  onMount(async () => {
    fetch(`https://xeno-canto.org/api/2/recordings?query=${sciName}`)
      .then((resp) => resp.json())
      .then((xenoRecordingData) => {
...
<h1 class={"text-center text-4xl " + audioRecordingURL ? "ml-[25%]" : ""}>
  {name} (<em> {sciName}</em>)
</h1>
...
<div class="flex flex-row justify-center gap-2">
  {#if browser}
    <div
      class={audioRecordingURL
        ? "w-[250px] h-[250px] -mt-[100px]"
        : "w-[250px] h-[250px]"}
    >
          ...
          <div class="img-container">
            {#if loaded.includes(imageIndex)}
              <img
                ...
                src={imageSrc}
                alt={name ? `A photo of a ${name} bird.` : ""}
                width="200"
                height="200"
              />
            {/if}
  ...
  <div class="p-0 mt-0">
    ...
    {:else}
      <p class="mdc-typography--headline6 mb-8">
        Unfortunately, we do not have any description of this bird.
      </p>
    {/if}

    {#if nuthatchDataAvailable}
      <h1 class={audioRecordingURL ? quickFactsCSS + " ml-8" : quickFactsCSS}>
        Quick Facts
      </h1>
